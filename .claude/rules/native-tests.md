...
- **Structure**: `cpp/tests/support/` holds shared harness code (`HybridDatabaseHarness`, `TestDispatcher`, platform stub). Test files mirror `cpp/`'s own layout — `cpp/HybridSalveDatabase.*` (root) → `cpp/tests/*` (root), `cpp/database/*` → `cpp/tests/database/`, `cpp/query/*` → `cpp/tests/query/`, `cpp/sync/*` → `cpp/tests/sync/`.
