...
| **Query layer (ORM)** | CRUD básico (select/insert/update/delete, where/orderBy/limit/offset), inspirado no Drizzle. Ver [`query-layer.md`](./query-layer.md) | sem API de leitura/escrita, "banco" não tem como ser usado; é peça fundamental, não extra |
