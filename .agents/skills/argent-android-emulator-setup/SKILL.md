...
3. **Metro (for React Native)** — once a device is up, run `adb -s <serial> reverse tcp:<metro_port> tcp:<metro_port>` (default `8081`; use the resolved `metro_port` from `argent-environment-inspector` if the project configures a different one) so the device can reach Metro on your host. Repeat if the device restarts. See the `argent-metro-debugger` skill.
